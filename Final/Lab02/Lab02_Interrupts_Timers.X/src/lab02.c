...
void initialize_timer()
{
    ...
    CLEARBIT(IEC0bits.T3IE); // Enable Timer3 interrupt
...
void timer_loop()
{
    ...
    { 
        ...
        if (loopCount >= 2000)
        {
            ...
            // Display TMR3 counts and time in ms
            lcd_locate(0, 7);
            lcd_printf("c=%u, d=%.4fms", diff, msElapsed);
...
{ // invoked every 2 millisecond
    ...
    CLEARBIT(IFS0bits.T2IF); // Clear the interrupt flag
}
